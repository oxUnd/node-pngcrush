...
png_uint_32 measure_idats_binary(png_structp png_tr);
#ifdef PNGCRUSH_LOCO
...
{
    png_size_t check;
    user_png_structp image;

    image = (user_png_structp) png_get_io_ptr(png_ptr);

	if(image == NULL || image->data == NULL){
		png_error(png_ptr, "input stream is empty.");
		return;
	}

	check = image->length - image->current;
	...
	memcpy(data, &(image->data[image->current]), length);
	image->current += length;
}
...
png_defaultwrite_data(png_structp png_ptr, png_bytep data, png_size_t length)
{
   ...
   //    png_error(png_ptr, "Write Error");
    ...
    if(image->data == NULL){
      len = length;
      if((image->data = (png_bytep) malloc(len)) == NULL){
    ...
    memcpy(&(image->data[image->current]), data, length);
    image->current += length;
}
#else /* USE_FAR_KEYWORD */
/*
...
//#endif //end if 0
            ...
            idat_length[0] = measure_idats_bf(input_buffer);
            input_buffer->current = 0; //reset fp postion
//#if 0
//#ifdef PNGCRUSH_LOCO
//            if (new_mng)
//            {
...
//#else
                ...
                png_set_read_fn(read_ptr, (png_voidp) input_buffer,
                                (png_rw_ptr)pngcrush_default_read_data);
                // if (nosave == 0)
                //     png_init_io(write_ptr, fpout);
                if (nosave == 0)
                    png_set_write_fn(write_ptr, (png_voidp) out_buffer,
...
#  endif
 ...
 || (defined(PNG_READ_zTXt_SUPPORTED) && defined(PNG_WRITE_zTXt_SUPPORTED))
       ...
       /* (PNG_READ_zTXt_SUPPORTED and PNG_WRITE_zTXt_SUPPORTED) */
            ...
            //     idat_length[trial] = measure_idats_fp(fpin);
                idat_length[trial] = measure_idats_bf(out_buffer);
...
//             }
// #else
            input_length = (unsigned long) input_buffer->length;
            output_length = (unsigned long) out_buffer->length;
// #endif
            if (verbose > 0)
            {
    ...
    return out_buffer;  /* just in case */
...
int main(int argc, char *argv[]) {
    user_png_structp input_buffer, out_buffer;
    int read_step = 1024;
    int chunk_count = 0;
    int len = 0;
    int p_ptr = 0;
    ...
    memset(image_buffer, '\0', filesize);

    while ((len = fread(&image_buffer[(p_ptr++) * read_step], read_step, chunk_count, fpin))) {
    }

    fclose(fpin);

    input_buffer = (user_png_structp) malloc(sizeof(user_png_struct));
    ...
    fwrite(out_buffer->data, out_buffer->length, 1, fpin);
    fclose(fpin);
    free(out_buffer);
    free(input_buffer);
}
